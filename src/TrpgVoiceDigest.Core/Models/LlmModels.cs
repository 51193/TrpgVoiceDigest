    ...
    int CacheMissTokens = 0)
{
    public static LlmUsage? FromJsonElement(System.Text.Json.JsonElement? usageElement)
    {
        ...
        var cacheHit = usage.TryGetProperty("prompt_cache_hit_tokens", out var ht) && ht.TryGetInt32(out var hv) ? hv : 0;
        var cacheMiss = usage.TryGetProperty("prompt_cache_miss_tokens", out var mt) && mt.TryGetInt32(out var mv) ? mv : 0;
